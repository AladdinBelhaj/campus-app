...
4. Toujours dans `Gestion des utilisateurs`, créez un `RoleBinding` de type `Utilisateur` nommé `support-reader-auditor1` ayant comme objet `auditor1`.
5. Ouvrez `Administration -> Roles` pour verifier le role.
6. Ouvrez `RoleBindings` pour verifier le binding.
