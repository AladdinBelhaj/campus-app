# Lab09 — Simulation maintenance nœud Cordon / Drain / Remise en service sur OpenShift
...
| Droits requis | `cluster-admin` ou compte en full admin |
...
oc get pods -A -o wide --field-selector spec.nodeName=<nom-nœud>
...
- Notez le nom du nœud que vous allez isoler
...
# Retenter le drain avec seulement 2 réplicas
oc scale deployment webapp-test --replicas=2
oc adm drain <nom-nœud> --ignore-daemonsets --delete-emptydir-data
```
...
```bash
# Option 1 : augmenter les réplicas avant le drain
oc scale deployment webapp-test --replicas=4

# Option 2 : forcer (attention en production !)
oc adm drain <nom-nœud> \
  --ignore-daemonsets \
  --delete-emptydir-data \
  --disable-eviction
...
- Comprendre l'impact d'un PDB sur la procédure de maintenance
- Savoir adapter la stratégie selon les workloads
...
oc get pods -A -o wide \
  --field-selector spec.nodeName=<nom-nœud>
...
Les deux sont équivalents pour 3 réplicas, mais la sémantique diffère selon les cas.
