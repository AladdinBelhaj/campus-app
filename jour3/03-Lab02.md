...
- référence au Secret `docushare-db-secret`
- une sauvegarde automatique chaque jour à `1h00`
- stockage des sauvegardes sur un `PVC` nommé `docushare-db-backup`
- créez d’abord un `PVC` nommé `docushare-db-backup`
- puis référencez ce `PVC` dans `spec.backup.pvcName`
...
- un pod PostgreSQL ;
- un `PVC` de données créé automatiquement ;
- un service PostgreSQL ;
- un mécanisme de sauvegarde automatique chaque jour à `1h00`, stocké sur le volume `docushare-db-backup`.


<details>
<summary>💡 Hint - PVC Backup </summary>

```yaml
apiVersion: v1
kind: PersistentVolumeClaim
metadata:
  name: docushare-db-backup
spec:
