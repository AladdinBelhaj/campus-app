...
- la difference entre le stockage AWS de base et les classes exposées par Ceph ;
- l'installation de l'opérateur ODF ;
- le déploiement du cluster de stockage ODF ;
...
Sur ce cluster, Ceph n'est pas déployé sur des disques locaux comme sur certaines variantes bare metal.
...
7. saisir l'ARN, exemple attendu : `arn:aws:iam::809747278553:role/tf-rosa-classic/atelier-rosa-classic-odf-operator-role`
8. laissez activée l'option `Console plugin` ;
...
- l'opérateur ODF apparait dans `Installed Operators` ;
- le namespace `openshift-storage` existe ;
- le menu `Storage -> Data Foundation` devient disponible (après environ 30 min;

### Ce qu'il faut retenir

- l'opérateur ne fournit pas encore le stockage ;
...
![configure.png](assets/configure.png)

Choix recommandés pour ce lab :
...
- options avancées : laissez les options facultatives désactivées
![img_8.png](assets/img_8.png)
- capacité demandée : `1 TiB`
- selection des noeuds : choisissez les trois workers
  ![img_9.png](assets/img_9.png)
- profil : `Lean mode`
  ![img_10.png](assets/img_10.png)

- chiffrement et options réseau avancées : laissez désactivé pour le lab
...
Il est recommandé d'attendre jusqu'a voir un événement du type :

`successfully configured CephCluster "openshift-storage/ocs-storagecluster-cephcluster"`

![img_16.png](assets/img_16.png)

Puis de vérifier l'apparition des `StorageClass` ODF/Ceph :
...
- À cette étape, ODF ne se contente plus d'installer un opérateur ;
- Il déploie le vrai plan de stockage qui sera consommé par les applications.

## Etape 5 - Verifier la santé de ODF et de Ceph
...
- un meme `PVC` ;
- monte dans deux pods différents ;
- avec une écriture lisible depuis le second pod.

C'est ce qui démontre le comportement `ReadWriteMany`.

</details>

### Ce que vous devez trouver

- le `PVC` `RWX` est monté par deux pods ;
- le second pod lit le fichier écrit par le premier ;
