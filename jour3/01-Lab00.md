...
- ce que change un cluster `ROSA Classic` pour le stockage ;
...
- le nettoyage du namespace de lab.
...
- vous etes connecte au cluster avec un compte `cluster-admin` pour les etapes 1 a 6 ;
- vous pouvez utiliser `oc` depuis votre poste ;
- le cluster dispose d'au moins trois noeuds worker ou infra eligibles pour ODF ;
- la classe AWS `gp3-csi` ou `gp2-csi` existe deja ;
- les comptes `participant1` et `participant2` sont deja prets.

## Recuperer l'acces `oc` si besoin

Si vous venez de creer le cluster avec le socle Terraform du depot :

Windows / PowerShell :

```powershell
$ApiUrl = & terraform.exe '-chdir=./terraform-rosa-classic' output -raw api_url
$User = & terraform.exe '-chdir=./terraform-rosa-classic' output -raw cluster_admin_username
$Pass = & terraform.exe '-chdir=./terraform-rosa-classic' output -raw cluster_admin_password
oc login $ApiUrl --username $User --password $Pass
```

Linux / bash :

```bash
API_URL="$(terraform -chdir=./terraform-rosa-classic output -raw api_url)"
USER_NAME="$(terraform -chdir=./terraform-rosa-classic output -raw cluster_admin_username)"
PASSWORD="$(terraform -chdir=./terraform-rosa-classic output -raw cluster_admin_password)"
oc login "$API_URL" --username "$USER_NAME" --password "$PASSWORD"
```
...
1. verifier le cluster et les classes de stockage AWS disponibles ;
2. verifier les prerequis ODF sur le cluster ;
3. installer l'operateur ODF ;
4. deployer le cluster de stockage ODF ;
5. verifier la sante des composants ODF / Ceph ;
6. lire les nouvelles `StorageClass` ;
7. realiser un exercice `RBD / RWO` ;
8. verifier la persistance apres recreation d'un pod ;
9. realiser un exercice `CephFS / RWX` ;
...
Commencez par verifier le contexte courant :
...
Installez l'operateur depuis la console OpenShift.
...
## Etape 8 - Recreer un pod et relire la donnee
...
<details>
<summary>Hint - Que prouve cette etape ?</summary>
...
- la donnee ecrite avant la suppression du pod est toujours lisible.
...
- la persistance est donc portee par le volume.
